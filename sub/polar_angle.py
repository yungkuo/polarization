...
for i in range(8): 
    ...
    theta1 = np.linspace(0,theta1max,1000*i)
    #def theta2(theta1,n1,n2):
    #    return np.arcsin(n1*np.sin(theta1)/n2)
    theta2 = asin(n1*sin(theta1)/n2)
    
    def delta(n1=n1,d=d,theta1=theta1,wavelength=wavelength):
    ...
    def integrandA(theta1=theta1,D=D,nj=nj,theta2=theta2,fs=fs,fpb=fpb):
        return D**2*np.pi/4*nj*cos(theta2)/cos(theta1)*(np.abs(fs-cos(theta1)*fpb))**2*sin(theta1)
    def integrandB(theta1=theta1,D=D,nj=nj,theta2=theta2,fpa=fpa):
        return D**2*np.pi*nj*cos(theta2)/cos(theta1)*(np.abs(fpa))**2*sin(theta1)**3
    def integrandC(theta1=theta1,D=D,nj=nj,theta2=theta2,fpa=fpa,fs=fs):
    ...
    A = np.nansum(integrandA(theta1,D,nj,theta2,fs,fpb)*(theta1max-0)/1000*i)
    B = np.nansum(integrandB(theta1,D,nj,theta2,fpa)*(theta1max-0)/1000*i)
    C = np.nansum(integrandC(theta1,D,nj,theta2,fpa,fs)*(theta1max-0)/1000*i)
    
       
    '''  
    A = -1*np.diff(quad(integrandA, 0, theta1max))
