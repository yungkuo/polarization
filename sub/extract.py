...
def blinkon_mean(mov, pts, scan, fig, ax):
    ...
    #    fig.canvas.draw()         
        ...
        #ax[n].plot(np.arange(0,frame,1), np.tile(threshold[n], frame), 'r-', label='threshold')
...
def bg_mean(mov, pts, scan, displacement, fig, ax):
    frame = len(mov[:,0,0])      
    timetraces1 = []
    for i in range(len(pts)):
        timetrace = np.mean(np.mean(mov[:, pts[i,1]-scan+displacement:pts[i,1]+scan+displacement, pts[i,0]-scan:pts[i,0]+scan],axis=1), axis=1)
        timetraces1 = np.append(timetraces1, timetrace)
    timetraces1 = np.reshape(timetraces1, [len(pts), frame])
        
   
    bg_std = np.std(timetraces1[:,:],axis=1,ddof=1,dtype='d')
    bg_intensity = np.mean(timetraces1[:,:],axis=1,dtype='d')
        
    return bg_intensity, bg_std
